...
createApp({
  setup() {
    ...
    const activeName = computed(() => {
      stateTick.value;
      return (
        state.playerNames[activePlayer.value] || `P${activePlayer.value + 1}`
      );
    });

    function doRoll() {
      // Server validates turn and advances to next automatically.
      wire.rollDice(activePlayer.value, {
        autoMove: autoMove.value,
        requireTurn: true,
      });
    }
    function drawChance() {
      wire.drawCard("chance", activePlayer.value);
    }
    function drawChest() {
      wire.drawCard("chest", activePlayer.value);
    }
    function toggleForceDoubles(ev) {
      wire.setForceDoubles(!!ev?.target?.checked);
    }
    ...
    function markCascade(i) {
      ...
      if (isRR(prop)) {
        PROPS.forEach((p, idx) => {
          if (isRR(p)) markPropertyDirty(idx);
        });
      } else if (isUtil(prop)) {
        PROPS.forEach((p, idx) => {
          if (isUtil(p)) markPropertyDirty(idx);
        });
    ...
    function pIndex(boardIndex) {
      return boardIndexToPropIndex[boardIndex];
    }
    function propName(boardIndex) {
    ...
    const colors = COLORS;
    function groupOf(boardIndex) {
      const i = pIndex(boardIndex);
      return PROPS[i].group;
    }
    ...
    function cardPayload(i) {
      ...
      if (s.owner === 0) {
        ...
        const owned = PROPS.reduce(
          (n, pp, ii) =>
            n +
            (pp.type === "rr" && state.properties[ii].owner === s.owner
              ? 1
              : 0),
          0
        );
        ...
        L2 = "RENT";
        L3 = "$" + r;
        L4 = s.hotel > 0 ? "H 0 HT 1" : "H " + s.houses + " HT 0";
      }
      return {
        lines: { L1, L2, L3, L4 },
    ...
    function onHousesChange(i) {
      ...
      draft.hotel = 0;
      wire.updateProperty(i, {
        houses: draft.houses,
        hotel: 0,
        override: gmOverride.value,
      });
    }
    ...
    function infoLine(i) {
      ...
      const mono =
        s.houses === 0 &&
        s.hotel === 0 &&
        groupMembers(p.group).every(
          (j) => state.properties[j].owner === s.owner
        )
          ? " (Monopoly×2)"
          : "";
    ...
    function bump(i, d) {
      setPos(i, (state.playerPos[i] + d + 40) % 40);
    }

    function resetAll() {
      if (!confirm("Reset all properties, names, positions, dice, and roster?"))
        return;
      wire.resetAll();
    }
    ...
    function enableAll() {
      for (let i = 0; i < 6; i++) {
        localEnabled.value[i] = true;
        wire.setPlayerEnabled(i, true);
      }
    }
    ...
    return {
      ...
      setPos,
      bump,
      isStreet: (b) => isStreet(PROPS[pIndex(b)]),
      // dice + turn
      autoMove,
      forceDoubles,
      toggleForceDoubles,
